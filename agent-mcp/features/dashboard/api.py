...
async def fetch_graph_data_logic(
    ...
    try:
        ...
        for row in task_rows:
            ...
            # task_node_map[task_id_val] = node_id_str
                    ...
                    for dep_task_id in depends_task_ids:
                        ...
                        if dep_node_id_str in node_ids and node_id_str in node_ids: # Ensure both nodes exist
                            edges.append({
                                ...
                                'to': node_id_str,
                                'title': f'Depends on {dep_task_id}',
                                'color': {'color': '#AAAAAA', 'opacity': 0.5}, # Lighter grey
                                'dashes': [3, 3], # Dashed line for dependency
...
async def fetch_task_tree_data_logic() -> Dict[str, List[Dict[str, Any]]]:
    ...
    try:
        ...
        cursor.execute("SELECT task_id, title, status, depends_on_tasks, description, created_at FROM tasks ORDER BY created_at ASC")
        task_rows = cursor.fetchall()
                    ...
                    for dep_task_id in depends_task_ids:
                        ...
                        if dep_node_id_str in node_ids and node_id_str in node_ids:
                            edges.append({
                                'from': dep_node_id_str,
                                'to': node_id_str,
                                'arrows': {'to': {'enabled': True, 'scaleFactor': 0.8}}, # Make arrows clear
