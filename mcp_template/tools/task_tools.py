...
async def assign_task_tool_impl(arguments: Dict[str, Any]) -> List[mcp_types.TextContent]:
    ...
    try:
        ...
        # Generate task ID and timestamps (main.py:1349-1351)
        ...
        task_data_for_db = { # main.py:1354-1367
            ...
            "depends_on_tasks": json.dumps(depends_on_tasks_list or []),
            "notes": json.dumps([])
        }
        ...
        task_data_for_memory["depends_on_tasks"] = depends_on_tasks_list or []
        ...
        return [mcp_types.TextContent(
            type="text",
            text=f"Task '{new_task_id}' assigned to agent '{target_agent_id}'.\nTitle: {task_title}"
        )]
...
async def create_self_task_tool_impl(arguments: Dict[str, Any]) -> List[mcp_types.TextContent]:
    ...
    # Hierarchy Validation (main.py:1426-1431)
    if requesting_agent_id != "admin" and actual_parent_task_id is None:
        logger.warning(f"Agent '{requesting_agent_id}' attempted to create a root task implicitly.")
        return [mcp_types.TextContent(
            type="text",
            text="Error: Non-admin agents cannot create root tasks implicitly. Please use 'view_tasks' to find an appropriate parent task ID and provide 'parent_task_id'."
        )]
    ...
    try:
        ...
        new_task_id = _generate_task_id() # main.py:1434
        created_at_iso = datetime.datetime.now().isoformat()
        status = "pending"
        ...
        task_data_for_db = { # main.py:1439-1452
            ...
            "created_at": created_at_iso,
            "updated_at": created_at_iso,
            "parent_task": actual_parent_task_id,
            "child_tasks": json.dumps([]),
            "depends_on_tasks": json.dumps(depends_on_tasks_list or []),
            "notes": json.dumps([])
        }
        ...
        return [mcp_types.TextContent(
            type="text",
            text=f"Self-assigned task '{new_task_id}' created.\nTitle: {task_title}"
        )]
...
def register_task_tools():
    register_tool(
        ...
        input_schema={ # From main.py:1701-1724
            ...
            "properties": {
                ...
                "parent_task_id": { # Added this from original assign_task_tool call
                    "type": "string", "description": "ID of the parent task (optional)"
                }
            },
    ...
    register_tool(
        ...
        description="Agent tool to create a task for themselves, optionally specifying dependencies and a parent task.",
        input_schema={ # From main.py:1727-1750
            ...
            "properties": {
                ...
                "parent_task_id": { # Added this from original create_self_task_tool call
                    "type": "string", "description": "ID of the parent task (optional, defaults to agent's current task if applicable)"
                }
            },
