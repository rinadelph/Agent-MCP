...
def is_vss_loadable() -> bool:
    ...
    if not g.global_vss_load_tested:
        ...
        logger.warning("is_vss_loadable() called before initial VSS loadability check. Performing check now.")
...
def check_vss_loadability() -> bool:
    ...
    if sqlite_vec is None:
        logger.error("sqlite-vec library is not installed. RAG functionality will be disabled.")
    ...
    try:
        ...
        sqlite_vec.load(temp_conn) # Call the load function from the imported sqlite_vec
        ...
        logger.error("Installed sqlite3 version doesn't support enable_load_extension. sqlite-vec cannot be loaded.")
        g.global_vss_load_successful = False
    except sqlite3.Error as e: # Catch sqlite3 specific errors during load
        logger.error(f"SQLite error during initial check for sqlite-vec: {e}", exc_info=True)
        g.global_vss_load_successful = False
        ...
        logger.error("RAG functionality will be disabled. Ensure 'sqlite-vec' is installed and SQLite supports extensions.")
        g.global_vss_load_successful = False
    finally:
        if temp_conn:
            temp_conn.close()
...
def get_db_connection() -> sqlite3.Connection:
    ...
    try:
        ...
        logger.error(f"Failed to create directory for database at {db_file_path.parent}: {e}")
    ...
    try:
        ...
        if g.global_vss_load_successful and sqlite_vec:
            try:
                ...
                logger.warning("This sqlite3 connection instance does not support enable_load_extension, or sqlite_vec.load failed.")
                ...
                logger.error(f"SQLite error loading sqlite-vec for new connection: {e_load}")
            except Exception as e_load_ext: # From main.py:232 (original line numbers)
                logger.error(f"Failed to load sqlite-vec for new connection: {e_load_ext}")
            finally:
                # Always disable extension loading after attempting, regardless of success.
                # From main.py:235-238 (original line numbers)
                try:
                    conn.enable_load_extension(False)
                except (sqlite3.Error, AttributeError): # Catch errors if disabling also fails or not supported
                    pass
        else:
            if sqlite_vec: # Only log this if sqlite_vec was imported but not deemed loadable
                logger.debug("sqlite-vec extension not loaded for this connection (globally not loadable or library not found).")


    except AttributeError as e_attr: # From main.py:240, if sqlite3 itself is too old for enable_load_extension
        logger.error(f"The sqlite3 library version does not support enable_load_extension: {e_attr}. sqlite-vec cannot be used.")
        if conn:
            conn.close()
        # This is a critical issue if VSS is expected.
        raise RuntimeError("SQLite version does not support extension loading.") from e_attr
    except sqlite3.OperationalError as e_op: # More specific error for DB file issues
        logger.error(f"SQLite OperationalError connecting to DB at '{db_file_path}': {e_op}", exc_info=True)
        if conn:
            conn.close()
        raise RuntimeError(f"Failed to connect to database: {e_op}") from e_op
    except sqlite3.Error as e_sql: # From main.py:244 (original line numbers) - general SQLite errors
        logger.error(f"SQLite error connecting to or setting up DB connection at '{db_file_path}': {e_sql}", exc_info=True)
        if conn:
            conn.close()
        raise RuntimeError(f"Database connection error: {e_sql}") from e_sql # Re-raise as a more generic runtime error
    except Exception as e_unexpected: # From main.py:248 (original line numbers) - other unexpected errors
        logger.error(f"Unexpected error getting DB connection for '{db_file_path}': {e_unexpected}", exc_info=True)
        if conn:
            conn.close()
        raise RuntimeError(f"Unexpected database connection error: {e_unexpected}") from e_unexpected
