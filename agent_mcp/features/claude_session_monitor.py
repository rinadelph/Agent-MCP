...
class ClaudeSessionMonitor:
    ...
    async def register_new_session(self, session_id: str, session_data: Dict[str, Any]):
        ...
        try:
            ...
            logger.info(f"New Claude Code session detected: {session_id} (PID: {session_data.get('pid')})")
            
        except Exception as e:
            logger.error(f"Error registering new session {session_id}: {e}", exc_info=True)
    
    async def update_session_activity(self, session_id: str, session_data: Dict[str, Any]):
        ...
        try:
            ...
            cursor.execute("""
                ...
                WHERE session_id = ?
            """, (
                session_data.get('last_activity', datetime.datetime.now().isoformat()),
                json.dumps(session_data),
                session_id
            ))
            ...
            logger.error(f"Error updating session activity {session_id}: {e}", exc_info=True)
    ...
    async def mark_session_inactive(self, session_id: str):
        ...
        try:
            ...
            logger.error(f"Error marking session inactive {session_id}: {e}", exc_info=True)
...
async def run_claude_session_monitoring(interval: int = 5):
