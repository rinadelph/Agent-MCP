        ...
        if is_vss_loadable(): # Check global VSS status
            try:
                ...
                if cursor.fetchone() is not None:
                    ...
                    # Note: vec0 requires either 'LIMIT 10' (literal) or 'AND k = ?' (parameter)
                    # Using k = ? since we want to parameterize the limit
                    sql_vector_search = """
...
async def query_rag_system_with_model(
    ...
    try:
        ...
        if is_vss_loadable():
            # Check if rag_embeddings virtual table exists
            cursor.execute("SELECT name FROM sqlite_master WHERE type IN ('table', 'virtual') AND name='rag_embeddings'")
            if cursor.fetchone() is not None:
                ...
                # Convert embedding to JSON string for sqlite-vec MATCH clause
                # This is the correct, safe way to pass vectors to sqlite-vec
                query_embedding_json = json.dumps(query_embedding)
                ...
                vector_search_sql = """
                    ...
                    JOIN rag_chunks c ON e.rowid = c.chunk_id
