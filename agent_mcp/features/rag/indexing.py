...
async def _get_embeddings_batch_openai(
    ...
    try:
        ...
        logger.error(f"OpenAI embedding API error in batch starting at {batch_index_start}: {e}")
...
async def run_rag_indexing_periodically(interval_seconds: int = 300, *, task_status=anyio.TASK_STATUS_IGNORED) -> NoReturn:
    """
    ...
    # Get the API key directly from environment instead of using cached value
    ...
    while g.server_running: # Uses global flag (main.py:521)
        ...
        logger.info("Starting RAG index update cycle (indexing markdown, context, and tasks)...")
        ...
        try:
            ...
            if not is_vss_loadable(): # This checks the global flag set by initial check
                 logger.warning("Vector Search (sqlite-vec) is not loadable. Skipping RAG indexing cycle.")
            ...
            if cursor.fetchone() is None:
                logger.warning("Vector table 'rag_embeddings' not found. Skipping RAG indexing cycle. Ensure DB schema is initialized.")
            ...
            rag_meta_data = {row['meta_key']: row['meta_value'] for row in cursor.fetchall()}
            ...
            # Code files are not indexed in simple mode
            ...
            for md_path_obj in all_md_files_found:
                try:
                    ...
                    sources_to_check.append(('markdown', normalized_path, content, mod_time, current_hash))
            ...
            max_ctx_mod_time_iso = last_ctx_time_str # Keep as ISO string for direct comparison
            ...
            for row in cursor.fetchall():
                ...
                content_for_embedding = f"Context Key: {key}\nDescription: {desc}\nValue: {value_str}"
                current_hash = hashlib.sha256(content_for_embedding.encode('utf-8')).hexdigest()
                sources_to_check.append(('context', key, content_for_embedding, last_mod_iso, current_hash))
            ...
            # 4. Scan Tasks (for System 8)
            max_task_mod_time_iso = last_indexed_timestamps.get('last_indexed_tasks', '1970-01-01T00:00:00Z')
            last_task_time_str = last_indexed_timestamps.get('last_indexed_tasks', '1970-01-01T00:00:00Z')
            ...
            for task_row in cursor.fetchall():
                ...
                sources_to_check.append(('task', task_id, content_for_embedding, last_mod_iso, current_hash))
            ...
            sources_to_process_for_embedding: List[Tuple[str, str, str, str]] = [] # type, ref, content, current_hash
            for source_type, source_ref, content, _, current_hash in sources_to_check:
                ...
                if current_hash != stored_source_hash:
                    logger.info(f"Change detected for {source_type}: {source_ref} (Hash mismatch or new). Queued for re-indexing.")
                    sources_to_process_for_embedding.append((source_type, source_ref, content, current_hash))
                # else: logger.debug(f"No change for {source_type}:{source_ref} (hash match)")

            if not sources_to_process_for_embedding:
                logger.info("No new or modified sources found requiring RAG index update.")
            else:
                logger.info(f"Processing {len(sources_to_process_for_embedding)} updated/new sources for RAG index.")
                ...
                logger.info("Deleting existing chunks and embeddings for sources needing update...")
                delete_count = 0
                ...
                if delete_count > 0:
                    logger.info(f"Deleted {delete_count} old chunks and their embeddings.")
                ...
                for source_type, source_ref, content, current_hash_of_source in sources_to_process_for_embedding:
                    ...
                    # Simple mode: Use appropriate chunking based on type
                    if source_type == 'markdown':
                        ...
                        # Simple chunking for other types (context, tasks)
                ...
                if all_chunks_texts_to_embed:
                    ...
                    all_embeddings_vectors: List[Optional[List[float]]] = [None] * len(all_chunks_texts_to_embed)
                    embeddings_api_successful = True # Flag to track overall success of API calls
                    ...
                    for group_start_idx in range(0, len(all_chunks_texts_to_embed), MAX_CONCURRENT_EMBEDDING_REQUESTS * PARALLEL_EMBEDDING_BATCH_SIZE):
                        # Determine how many batches to run in this parallel group
                        num_batches_in_group = 0
                        temp_idx = group_start_idx
                        while num_batches_in_group < MAX_CONCURRENT_EMBEDDING_REQUESTS and temp_idx < len(all_chunks_texts_to_embed):
                            num_batches_in_group += 1
                            temp_idx += PARALLEL_EMBEDDING_BATCH_SIZE
                        
                        logger.info(f"Processing up to {num_batches_in_group} embedding batches in parallel (group starting at chunk {group_start_idx})...")
                        
                        try:
                            async with anyio.create_task_group() as tg_embed:
                                for i in range(num_batches_in_group):
                                    batch_actual_start_index = group_start_idx + i * PARALLEL_EMBEDDING_BATCH_SIZE
                                    if batch_actual_start_index >= len(all_chunks_texts_to_embed):
                                        break # No more chunks
                                    
                                    batch_end_index = min(batch_actual_start_index + PARALLEL_EMBEDDING_BATCH_SIZE, len(all_chunks_texts_to_embed))
                                    current_batch_chunks = all_chunks_texts_to_embed[batch_actual_start_index:batch_end_index]
                                    
                                    if not current_batch_chunks: continue
                            ...
                            embeddings_api_successful = False # Mark failure if task group fails
                        
                        if not embeddings_api_successful: break # Stop if a task group failed

                        # Minimal delay between batch groups (Original main.py:689)
                        if group_start_idx + MAX_CONCURRENT_EMBEDDING_REQUESTS * PARALLEL_EMBEDDING_BATCH_SIZE < len(all_chunks_texts_to_embed):
                            await anyio.sleep(0.1) # Reduced from 0.2

                    embedding_api_duration = time.time() - embedding_api_call_start_time
                    logger.info(f"Completed all embedding API calls in {embedding_api_duration:.2f} seconds.")

                    # Check for failed embeddings (None values)
                    failed_embedding_count = sum(1 for emb_vec in all_embeddings_vectors if emb_vec is None)
                    if failed_embedding_count > 0:
                        logger.warning(f"{failed_embedding_count} out of {len(all_embeddings_vectors)} embeddings failed to generate.")
                        # If a significant portion failed, mark the overall API call as unsuccessful
                        if failed_embedding_count > len(all_embeddings_vectors) // 2: # More than half failed
                            embeddings_api_successful = False
                            logger.error("More than half of the embeddings failed. Marking RAG indexing cycle for these sources as unsuccessful.")
                    
                    # Insert new chunks and embeddings into DB (Original main.py:697-722)
                    if embeddings_api_successful:
                        logger.info("Inserting new chunks and embeddings into the database...")
                        inserted_count = 0
                        indexed_at_iso = datetime.datetime.now().isoformat()
                        for i, chunk_text_to_insert in enumerate(all_chunks_texts_to_embed):
                            ...
                            try:
                                ...
                                meta_key_for_hash_update = f"hash_{source_type}_{source_ref}"
                                processed_hashes_to_update_in_meta[meta_key_for_hash_update] = current_hash_of_source
                                ...
                                logger.error(f"Unexpected error inserting chunk/embedding: {e_ins}", exc_info=True)

                        logger.info(f"Successfully inserted {inserted_count} new chunks/embeddings.")
                        ...
                        if processed_hashes_to_update_in_meta:
                            logger.info(f"Updating {len(processed_hashes_to_update_in_meta)} source hashes in rag_meta...")
                            meta_update_tuples = list(processed_hashes_to_update_in_meta.items())
                            cursor.executemany("INSERT OR REPLACE INTO rag_meta (meta_key, meta_value) VALUES (?, ?)", meta_update_tuples)
                    else:
                        logger.warning("Skipping DB insertion and hash updates for this RAG cycle due to embedding API errors.")
            ...
            if 'embeddings_api_successful' not in locals() or embeddings_api_successful: # Check if flag exists and is True
                ...
                # Update tasks timestamp (tasks are indexed in simple mode)
                ...
                logger.warning("Skipping rag_meta timestamp updates due to errors in the embedding/indexing cycle.")
            ...
            try:
                ...
                logger.info(f"DB RAG DIAGNOSTIC: Found {chunk_count_diag} chunks and {embedding_count_diag} embeddings post-cycle.")
            ...
            if "no such module: vec0" in str(e_sqlite_op) or "vector search requires" in str(e_sqlite_op).lower():
                logger.warning(f"Vector search module (vec0) not available or table missing. RAG indexing cycle skipped. Error: {e_sqlite_op}")
                g.global_vss_load_successful = False # Mark VSS as not usable if this happens
            else:
                logger.error(f"Database operational error in RAG indexing cycle: {e_sqlite_op}", exc_info=True)
        ...
        logger.info(f"RAG index update cycle finished in {elapsed_cycle_time:.2f} seconds.")
    ...
    try:
        ...
        for task_row in tasks:
            ...
            if task_data.get('depends_on_tasks'):
                try:
                    task_data['depends_on_tasks'] = json.loads(task_data['depends_on_tasks'])
