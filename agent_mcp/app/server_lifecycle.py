...
async def application_startup(project_dir_path_str: str, admin_token_param: Optional[str] = None):
    """
    ...
    try:
        ...
        if admin_token_param:
            ...
            cursor.execute("""
                ...
                VALUES (?, ?, ?, ?, ?)
            """, (admin_token_key_in_db, json.dumps(effective_admin_token), 
                  datetime.datetime.now().isoformat(), "server_startup", "Persistent MCP Admin Token"))
            conn_admin_token.commit()
            logger.info(f"Using admin token provided via {token_source_description}.")
        else:
            cursor.execute("SELECT value FROM project_context WHERE context_key = ?", (admin_token_key_in_db,))
            row = cursor.fetchone()
            if row and row["value"]:
                try:
                    ...
                    if isinstance(loaded_token, str) and loaded_token:
                        ...
                        logger.info(f"Loaded admin token from {token_source_description}.")
                    else:
                        logger.warning("Stored admin token in DB is invalid. Generating a new one.")
                except json.JSONDecodeError:
                    logger.warning("Failed to decode stored admin token from DB. Generating a new one.")
        ...
        logger.error(f"Database error during admin token persistence: {e_sql_admin}. Falling back to temporary token.", exc_info=True)
        ...
        logger.error(f"Unexpected error during admin token persistence: {e_admin}. Falling back to temporary token.", exc_info=True)
        g.admin_token = admin_token_param if admin_token_param else generate_token()
    ...
    try:
            ...
            for field_key in ["child_tasks", "depends_on_tasks", "notes"]:
                if isinstance(row_dict.get(field_key), str):
                    try:
                        ...
                        logger.warning(f"Failed to parse JSON for field '{field_key}' in task '{task_id_val}'. Defaulting to empty list.")
                        row_dict[field_key] = []
        ...
        g.file_map.clear()
        g.audit_log.clear()
        logger.info("In-memory file map and audit log initialized as empty for this session.")

    except sqlite3.Error as e_sql_load:
        logger.error(f"Database error during state loading: {e_sql_load}. Server might operate with incomplete state.", exc_info=True)
        # Decide if this is critical. Original proceeded with empty state.
        g.active_agents.clear()
        g.tasks.clear()
        g.agent_working_dirs.clear()
    except Exception as e_load:
        logger.error(f"Unexpected error during state loading: {e_load}. Exiting.", exc_info=True)
    ...
    if not initialize_openai_client(): # external.openai_service.initialize_openai_client
        logger.warning("OpenAI client failed to initialize. OpenAI-dependent features (like RAG) will be unavailable.")
    ...
    if g.global_vss_load_successful:
        ...
        logger.warning("sqlite-vec (VSS) extension is NOT loadable. RAG search functionality will be impaired.")
...
async def start_background_tasks(task_group: anyio.abc.TaskGroup):
    ...
    g.rag_index_task_scope = await task_group.start(run_rag_indexing_periodically, rag_interval)
    logger.info(f"RAG indexing task started with interval {rag_interval}s.")
    # Add other background tasks here if any
