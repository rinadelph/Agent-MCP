...
def _normalize_filepath(filepath_arg: str, agent_id_for_wd: Optional[str]) -> str:
    ...
    if not os.path.isabs(filepath_arg):
        ...
        if agent_id_for_wd and agent_id_for_wd in g.agent_working_dirs:
            ...
            logger.warning(f"Agent '{agent_id_for_wd}' not found in agent_working_dirs for path resolution. Using CWD.")
        
        resolved_path = Path(working_dir) / filepath_arg
    else:
        resolved_path = Path(filepath_arg)
    
    return resolved_path.resolve().as_posix() # Resolve to absolute and normalize to POSIX


# --- view_file_metadata tool ---
# Original logic from main.py: lines 1503-1533 (view_file_metadata_tool function)
async def view_file_metadata_tool_impl(arguments: Dict[str, Any]) -> List[mcp_types.TextContent]:
    ...
    log_audit(requesting_agent_id, "view_file_metadata", 
    ...
    try:
        ...
        cursor = conn.cursor()
        # main.py:1521
        cursor.execute("SELECT metadata, updated_by, last_updated, content_hash FROM file_metadata WHERE filepath = ?", 
                       (normalized_filepath_str,))
        row = cursor.fetchone()
        if row: # main.py:1523-1525
            try:
                metadata_parsed = json.loads(row["metadata"]) # Metadata is stored as JSON string
            except json.JSONDecodeError:
                logger.warning(f"Failed to parse JSON metadata for file '{normalized_filepath_str}'. Raw: {row['metadata']}")
                metadata_parsed = {"error": "Could not parse stored metadata string.", "raw_value": row["metadata"]}
        ...
        logger.error(f"Database error viewing file metadata for '{normalized_filepath_str}': {e_sql}", exc_info=True)
        response_message = f"Database error viewing file metadata: {e_sql}"
    except json.JSONDecodeError as e_json: # Should be caught per-item, but as fallback (main.py:1532)
        logger.error(f"Error decoding JSON from file_metadata table for '{normalized_filepath_str}': {e_json}", exc_info=True)
        response_message = f"Error decoding stored file metadata."
    except Exception as e:
        logger.error(f"Unexpected error viewing file metadata for '{normalized_filepath_str}': {e}", exc_info=True)
...
async def update_file_metadata_tool_impl(arguments: Dict[str, Any]) -> List[mcp_types.TextContent]:
    admin_auth_token = arguments.get("token")
    filepath_arg = arguments.get("filepath")
    metadata_to_set = arguments.get("metadata") # This is a Dict[str, Any]

    if not verify_token(admin_auth_token, "admin"): # main.py:1539 (Restricted to admin)
        return [mcp_types.TextContent(type="text", text="Unauthorized: Admin token required for updating file metadata.")]
    ...
    if not filepath_arg or not isinstance(filepath_arg, str) or \
       metadata_to_set is None or not isinstance(metadata_to_set, dict):
        return [mcp_types.TextContent(type="text", text="Error: filepath (string) and metadata (dictionary) are required.")]

    # Resolve and normalize path (main.py:1545-1549)
    normalized_filepath_str = _normalize_filepath(filepath_arg, requesting_admin_id) # Use admin's context for WD if needed

    # Log audit (main.py:1551)
    log_audit(requesting_admin_id, "update_file_metadata", 
              {"filepath_normalized": normalized_filepath_str, "original_path": filepath_arg, "metadata_keys": list(metadata_to_set.keys())})
    ...
    try:
        ...
        logger.error(f"Metadata provided for file '{normalized_filepath_str}' is not JSON serializable: {e_type}")
        return [mcp_types.TextContent(type="text", text=f"Error: Provided metadata is not JSON serializable: {e_type}")]
    ...
    try:
        ...
        cursor.execute("""
            ...
            VALUES (?, ?, ?, ?)
        """, (normalized_filepath_str, metadata_json_str, updated_at_iso, requesting_admin_id))
        
        log_agent_action_to_db(cursor, requesting_admin_id, "updated_file_metadata",
                               details={'filepath': normalized_filepath_str, 'action': 'set/update'})
        conn.commit()
        
        logger.info(f"File metadata for '{normalized_filepath_str}' updated by '{requesting_admin_id}'.")
        ...
        logger.error(f"Database error updating file metadata for '{normalized_filepath_str}': {e_sql}", exc_info=True)
        return [mcp_types.TextContent(type="text", text=f"Database error updating file metadata: {e_sql}")]
    except Exception as e:
        if conn: conn.rollback()
        logger.error(f"Unexpected error updating file metadata for '{normalized_filepath_str}': {e}", exc_info=True)
        return [mcp_types.TextContent(type="text", text=f"Unexpected error updating file metadata: {e}")]
    finally:
        if conn:
            conn.close()
    ...
    register_tool(
        ...
        input_schema={ # From main.py:1842-1852
            ...
            "properties": {
                ...
                "filepath": {"type": "string", "description": "Path to the file (can be relative to agent's CWD or absolute)"}
            },
    ...
    register_tool(
        ...
        input_schema={ # From main.py:1855-1867
            ...
            "properties": {
                ...
                "filepath": {"type": "string", "description": "Path to the file (can be relative or absolute)"},
                "metadata": {"type": "object", "description": "A JSON object containing the metadata to set for the file."}
            },
