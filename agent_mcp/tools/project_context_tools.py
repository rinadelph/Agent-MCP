...
def _analyze_context_health(context_entries: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    for entry in context_entries:
        ...
        if last_updated:
            try:
                updated_time = datetime.datetime.fromisoformat(last_updated.replace('Z', '+00:00').replace('+00:00', ''))
    ...
    health_score = max(0, min(100, 
        100 - (stale_ratio * 40) - (error_ratio * 50) - (large_ratio * 10)
    ))
    ...
    return {
        ...
        "recommendations": _generate_context_recommendations(stale_count, json_errors, large_entries, total)
    }

def _generate_context_recommendations(stale_count: int, json_errors: int, large_entries: int, total: int) -> List[str]:
    """Generate actionable recommendations based on context health"""
    recommendations = []
    
    if json_errors > 0:
        recommendations.append(f"Fix {json_errors} JSON parsing errors using validate_context_consistency")
    
    if stale_count > total * 0.3:  # More than 30% stale
        recommendations.append(f"Review and update {stale_count} stale entries (30+ days old)")
    
    if large_entries > 0:
        recommendations.append(f"Consider breaking down {large_entries} large entries into smaller components")
    
    if total > 100:
        recommendations.append("Consider archiving old context entries to improve performance")
    
    if not recommendations:
        recommendations.append("Context health is excellent - no immediate action required")
...
async def view_project_context_tool_impl(arguments: Dict[str, Any]) -> List[mcp_types.TextContent]:
    ...
    show_stale_entries = arguments.get("show_stale_entries", False)  # Show entries older than 30 days
    include_backup_info = arguments.get("include_backup_info", False)  # Include backup status
    max_results = arguments.get("max_results", 50)  # Limit results
    sort_by = arguments.get("sort_by", "last_updated")  # Sort by: key, last_updated, size
    ...
    try:
        ...
        if context_key_filter:
            ...
            where_conditions.append("(context_key LIKE ? OR description LIKE ? OR value LIKE ?)")
        ...
        if not results_list:
            ...
            if show_health_analysis:
                ...
                cursor.execute("SELECT context_key, value, last_updated FROM project_context")
                ...
                response_parts.append(f"📊 **Context Health:** {health_icon} {health_status.title()} ({health_score}/100)")
                response_parts.append(f"   Total: {health_analysis['total']} entries")
                response_parts.append(f"   Issues: {health_analysis['json_errors']} JSON errors, {health_analysis['stale_entries']} stale, {health_analysis['large_entries']} large")
                
                if health_analysis["recommendations"]:
                    response_parts.append(f"   💡 {health_analysis['recommendations'][0]}")
                response_parts.append("")
            
            # Add backup info if requested
            if include_backup_info:
                response_parts.append("💾 **Backup Info:** Use bulk_update_project_context for backups")
                response_parts.append("")
            ...
            for i, entry in enumerate(results_list[:20]):  # Limit display to 20 entries
                ...
                response_parts.append(f"  Description: {entry.get('description', 'No description')}")
                response_parts.append(f"  Updated: {entry.get('last_updated', 'Unknown')} by {entry.get('updated_by', 'Unknown')}")
            ...
            if len(results_list) > 20:
                ...
                response_parts.append("Use max_results parameter to see more, or add filters to narrow results")
            
            # Add smart usage tips
            response_parts.append("\n💡 Smart Tips:")
            if not show_health_analysis:
                response_parts.append("• Add show_health_analysis=true for context health metrics")
            if not show_stale_entries:
                response_parts.append("• Add show_stale_entries=true to see entries needing updates")
            response_parts.append("• Use sort_by=[key|size|last_updated] for different sorting")
            response_parts.append("• Use validate_context_consistency to fix JSON errors")
...
async def backup_project_context_tool_impl(arguments: Dict[str, Any]) -> List[mcp_types.TextContent]:
    auth_token = arguments.get("token")
    backup_name = arguments.get("backup_name")  # Optional custom backup name
    include_health_report = arguments.get("include_health_report", True)  # Include health analysis in backup
    ...
    if not verify_token(auth_token, "admin"):
        return [mcp_types.TextContent(type="text", text="Unauthorized: Admin token required for backup operations")]

    log_audit(requesting_agent_id, "backup_project_context", {"backup_name": backup_name})
    ...
    try:
        ...
        if include_health_report and "health_report" in backup_data:
            ...
            health_icon = "🟢" if health["status"] == "excellent" else \
                         "🟡" if health["status"] == "good" else \
                         "🟠" if health["status"] == "needs_attention" else "🔴"
        ...
        log_agent_action_to_db(cursor, requesting_agent_id, "backup_project_context", 
                             backup_name, {"total_entries": backup_data["total_entries"], "backup_path": backup_path})
    ...
    register_tool(
        ...
        input_schema={
            ...
            "properties": {
                ...
                "show_health_analysis": {"type": "boolean", "description": "Include comprehensive health metrics and analysis (default: false)"},
                "show_stale_entries": {"type": "boolean", "description": "Show only entries older than 30 days needing review (default: false)"},
                "include_backup_info": {"type": "boolean", "description": "Include backup recommendations and info (default: false)"},
                
                # Display and sorting options
                "max_results": {"type": "integer", "description": "Maximum number of entries to return (default: 50)", "minimum": 1, "maximum": 200},
                "sort_by": {
                    "type": "string", "description": "Sort entries by specified field (default: last_updated)",
    ...
    register_tool(
        ...
        input_schema={
            ...
            "properties": {
                ...
                "backup_name": {"type": "string", "description": "Optional custom backup name (auto-generated if not provided)"},
                "include_health_report": {"type": "boolean", "description": "Include health analysis in backup (default: true)", "default": True}
            },
            "required": ["token"],
            "additionalProperties": False
        },
